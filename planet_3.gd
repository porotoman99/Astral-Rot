extends Edible
@onready var area2d = $"."
@onready var moon_orbit = $MoonOrbit
@onready var eaten = false
...
func _process(delta):
	area2d.rotation += 2.64 * delta
	if(is_instance_valid(moon_orbit)):
		moon_orbit.rotation += 0.321975666120738 * delta
	else:
		if(eaten):
...
func _on_body_entered(body):
	if(body.get_class() == "CharacterBody2D" and "balls" in body and eaten == false):
		body.add_size(value)
